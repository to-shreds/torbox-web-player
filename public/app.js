import { createDiscoveryUI } from './discover.js?v=restored11';
import { diagnosePlaybackFailure } from './playback-errors.js?v=restored11';
import { apiUrl, mediaUrl, apiMode, getSessionToken, setSessionToken, clearSessionToken, credentialsMode } from './runtime.js?v=restored11';
import { rememberApiKey, loadRememberedApiKey, forgetApiKey } from './vault.js?v=restored11';
import { createEncryptedTransfer, decryptEncryptedTransfer, applyTransferredState, transferLookup, transferCodeFromHash, buildTransferLink } from './device-transfer.js?v=restored11';
import { listRecent, continueWatchingItems, recordRecent, removeRecentTitle, recentForContext, resumePosition, formatResumeTime } from './history.js?v=restored11';
import { getSettings, saveSettings, resetSettings } from './settings.js?v=restored11';
import { rememberSourceSuccess, setAudioFeedback, setSourceBad, clearSourceMemory } from './source-memory.js?v=restored11';
import { clearSearchHistory } from './search-history.js?v=restored11';
import { hasParentPin, setParentPin, verifyParentPin, getKidProfile, updateKidProfile, resetKidAllowance, grantKidExtension, canStartKidPlayback, consumeKidPlayback, formatKidUsage } from './parental-controls.js?v=restored11';
...
function updatePlayerModeActions(){
  ...
  if(fs){fs.hidden=!playerFullscreenSupported();fs.textContent=playerFullscreen?'Exit full screen':fullscreenIntent?'Return to full screen':'Full screen';}
  if(pip){
    const supported=playerPiPSupported(video);pip.hidden=false;pip.disabled=!supported||!video||video.readyState===0;
    pip.textContent=supported?(document.pictureInPictureElement===video?'Exit picture in picture':'Picture in picture'):'PiP unavailable';
  }
}
...
$('toggle-fullscreen').addEventListener('click',async()=>{
  ...
  if(document.fullscreenElement===shell||document.fullscreenElement===video){fullscreenIntent=false;try{await document.exitFullscreen();}catch{}updatePlayerModeActions();return;}
