...
export function createApp({ env = process.env, provider, providerFactory, discoveryFetch = fetch, discoveryService, sourceLookupService, driveTransferService, now = Date.now } = {}) {
  ...
  const server = http.createServer(async (request, response) => {
    ...
    try {
      ...
      if (method === 'GET' && path === '/healthz') return json(response, 200, { ok: true, version: '2.2.1', stage: 'catalog-first-preview' });
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  server.listen(Number(process.env.PORT || 10000), '0.0.0.0', () => console.log(JSON.stringify({ event: 'listening', version: '2.2.1' })));
