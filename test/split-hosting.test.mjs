...
async function fixture(t) {
  ...
  const mediaFetch = async (_url, options = {}) => new Response(Buffer.from('V'), { status: options.headers?.Range ? 206 : 200, headers: { 'content-type': 'video/mp4', 'content-length': '1', 'accept-ranges': 'bytes', ...(options.headers?.Range ? { 'content-range': 'bytes 0-0/10' } : {}) } });
...
test('opaque media ticket works without a Render cookie and exposes range headers to GitHub Pages', async t => {
  const { call }=await fixture(t);
  const token=(await (await call('/api/login',{method:'POST',data:{password}})).json()).sessionToken;
  const play=await (await call('/api/playback',{method:'POST',token,data:{viewer:'viewer-1',videoId:'torrents:1:0'}})).json();
  assert.match(play.mediaUrl,/^\/media\/[A-Za-z0-9_-]{43}$/);
  const media=await call(play.mediaUrl,{headers:{Range:'bytes=0-0'}});
  assert.equal(media.status,206); assert.equal(await media.text(),'V');
  assert.equal(media.headers.get('access-control-allow-origin'),page);
  assert.ok(media.headers.get('access-control-expose-headers').includes('Content-Range'));
});
test('wrong existing Render session cannot borrow another session media ticket', async t => {
  const { call, sessions }=await fixture(t);
  const token=(await (await call('/api/login',{method:'POST',data:{password}})).json()).sessionToken;
  const play=await (await call('/api/playback',{method:'POST',token,data:{viewer:'viewer-1',videoId:'torrents:1:0'}})).json();
  const other=sessions.create();
  assert.equal((await call(play.mediaUrl,{headers:{Cookie:`tw_session=${other.id}`,Range:'bytes=0-0'}})).status,404);
});
test('bearer logout revokes both API session and its media tickets', async t => {
  const { call }=await fixture(t);
  const token=(await (await call('/api/login',{method:'POST',data:{password}})).json()).sessionToken;
  const play=await (await call('/api/playback',{method:'POST',token,data:{viewer:'viewer-1',videoId:'torrents:1:0'}})).json();
