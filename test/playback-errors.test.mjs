...
const relay = 'https://torbox-web-player-key.onrender.com/media/'+'a'.repeat(43);
...
test('only verified TorBox CDN URLs and opaque Render tickets are trusted media', async () => {
  for (const url of ['https://outside.example/video','http://store.tb-cdn.io/file','https://tb-cdn.io.evil.test/file','https://evil.test/media/'+'a'.repeat(43)]) assert.equal((await diagnosePlaybackFailure(url,4)).kind,'session');
