...
import { recentKey, normalizeRecent, resumePosition, recordRecent, listRecent, clearRecent, removeRecent, formatResumeTime } from '../public/history.js';
...
test('one recent item can be removed without clearing the rest',()=>{
  const store=memoryStore();
  recordRecent({title:'One',current:{type:'movie',id:'tt1000001'}},10,100,{store});
  recordRecent({title:'Two',current:{type:'movie',id:'tt1000002'}},20,100,{store});
  const rows=listRecent(store);assert.equal(rows.length,2);
