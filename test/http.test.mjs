...
test('signed-out browsers cannot access protected routes or media', async t => {
  const { call } = await fixture(t);
  for (const [path, method] of [['/api/library', 'GET'], ['/api/playback', 'POST'], ['/api/progress', 'PUT'], ['/api/owner/diagnostics', 'POST'], ['/media/not-a-ticket', 'GET']]) assert.equal((await call(path, { method, data: method === 'GET' ? undefined : {} })).status, 401);
});
...
test('private responses disable caching and frames', async t => {
  const { call } = await fixture(t), response = await call('/api/session'); assert.equal(response.headers.get('cache-control'), 'no-store, private'); assert.equal(response.headers.get('x-frame-options'), 'DENY'); assert.ok(response.headers.get('content-security-policy').includes("frame-ancestors 'none'")); assert.ok(response.headers.get('content-security-policy').includes("media-src 'self'"));
});
test('playback returns only an opaque same-origin media URL and progress resumes', async t => {
  const { call, login } = await fixture(t), auth = await login(); const start = async () => (await call('/api/playback', { ...auth, method: 'POST', data: { viewer: 'viewer-1', videoId: 'torrents:1:0' } })).json(); const one = await start(); assert.match(one.mediaUrl, /^\/media\/[A-Za-z0-9_-]{43}$/); assert.equal(one.delivery, 'relay'); assert.ok(!JSON.stringify(one).includes('fixture-secret')); assert.ok(!JSON.stringify(one).includes('tb-cdn')); const save = await call('/api/progress', { ...auth, method: 'PUT', data: { viewer: 'viewer-1', videoId: 'torrents:1:0', leaseId: one.leaseId, seq: 1, position: 45, duration: 100 } }); assert.equal((await save.json()).saved, true); assert.equal((await start()).progress.position, 45);
});
test('media relay forwards one byte range without exposing the upstream URL', async t => {
  ...
  const { call, login } = await fixture(t, { mediaFetch }), auth = await login(); const playback = await (await call('/api/playback', { ...auth, method: 'POST', data: { viewer: 'viewer-1', videoId: 'torrents:1:0' } })).json(); const media = await call(playback.mediaUrl, { cookie: auth.cookie, headers: { Range: 'bytes=0-0' } }); assert.equal(media.status, 206); assert.equal(await media.text(), 'Z'); assert.equal(media.headers.get('content-range'), 'bytes 0-0/100'); assert.equal(seen.range, 'bytes=0-0'); assert.ok(seen.url.includes('fixture-secret')); assert.ok(!media.url.includes('tb-cdn'));
});
test('media tickets cannot be used by a different household session', async t => {
  const { call, login } = await fixture(t), a = await login(), b = await login(); const playback = await (await call('/api/playback', { ...a, method: 'POST', data: { viewer: 'viewer-1', videoId: 'torrents:1:0' } })).json(); assert.equal((await call(playback.mediaUrl, { cookie: b.cookie, headers: { Range: 'bytes=0-0' } })).status, 404);
});
test('media relay rejects multipart ranges before contacting the provider', async t => {
  let calls = 0; const { call, login } = await fixture(t, { mediaFetch: async () => { calls++; return defaultMediaFetch('', { headers: { Range: 'bytes=0-0' } }); } }), auth = await login(); const playback = await (await call('/api/playback', { ...auth, method: 'POST', data: { viewer: 'viewer-1', videoId: 'torrents:1:0' } })).json(); assert.equal((await call(playback.mediaUrl, { cookie: auth.cookie, headers: { Range: 'bytes=0-1,4-5' } })).status, 416); assert.equal(calls, 0);
});
test('expired upstream link is renewed once and then relayed', async t => {
  let resolves = 0, fetches = 0; const providerExtra = { resolveForRelay: async videoId => ({ upstreamUrl: `https://store.tb-cdn.io/${++resolves === 1 ? 'expired' : 'fresh'}?token=fixture-secret`, file: { id: videoId } }) }; const mediaFetch = async url => { fetches++; if (String(url).includes('/expired')) return new Response('expired', { status: 403 }); return new Response(Buffer.from('R'), { status: 206, headers: { 'content-type': 'video/mp4', 'content-length': '1', 'accept-ranges': 'bytes', 'content-range': 'bytes 0-0/100' } }); };
  const { call, login } = await fixture(t, { providerExtra, mediaFetch }), auth = await login(); const playback = await (await call('/api/playback', { ...auth, method: 'POST', data: { viewer: 'viewer-1', videoId: 'torrents:1:0' } })).json(); const media = await call(playback.mediaUrl, { cookie: auth.cookie, headers: { Range: 'bytes=0-0' } }); assert.equal(media.status, 206); assert.equal(await media.text(), 'R'); assert.equal(resolves, 2); assert.equal(fetches, 2);
});
