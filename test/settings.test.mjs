...
test('settings default to simple family-friendly mode with smart automation',()=>{
  ...
  assert.equal(s.interfaceMode,'simple');assert.equal(s.resumeRewindSeconds,10);assert.equal(s.autoNext,true);assert.equal(s.autoNextDelaySeconds,8);assert.equal(s.creditsLeadSeconds,45);assert.equal(s.autoRecovery,true);
...
test('expanded settings persist allowed values and reject arbitrary values',()=>{
  ...
  const saved=saveSettings({interfaceMode:'full',resolution:'1080p',resumeRewindSeconds:15,autoNext:false,autoNextDelaySeconds:10,creditsLeadSeconds:60,autoRecovery:false,bufferSeconds:20,recentLimit:16,pauseOverlay:false,driveWatchOnly:false,driveDeleteMinutes:30,showCompletedRecent:true,cleanupCompletedEpisodes:false,playbackRate:1.5,sleepTimerMinutes:45,keepAwake:false,keyboardShortcuts:false,seekSeconds:30,showEpisodeProgress:false,rememberBrowse:false,sourceSizeProfile:'quality',showWatchlist:false,watchlistLimit:24,showNextUp:false,nextUpLimit:12,showSearchHistory:false,searchHistoryLimit:12,longPressShortcuts:false,showPlaybackHealth:false,autoLearnSources:false},store);
  assert.deepEqual(getSettings(store),saved);
  saveSettings({interfaceMode:'expert',resolution:'9999p',resumeRewindSeconds:999,bufferSeconds:1,recentLimit:100,autoNextDelaySeconds:77,creditsLeadSeconds:999,playbackRate:9,sleepTimerMinutes:7,seekSeconds:99,sourceSizeProfile:'huge',watchlistLimit:999,nextUpLimit:99,searchHistoryLimit:99,driveDeleteMinutes:999},store);
  const s=getSettings(store);
  for(const key of ['interfaceMode','resolution','resumeRewindSeconds','bufferSeconds','recentLimit','autoNextDelaySeconds','creditsLeadSeconds','playbackRate','sleepTimerMinutes','seekSeconds','sourceSizeProfile','watchlistLimit','nextUpLimit','searchHistoryLimit','driveDeleteMinutes'])assert.equal(s[key],DEFAULT_SETTINGS[key],key);
});
