...
function suggestionMetas(data,type,query){
  ...
  for(const row of data.d.slice(0,40)){
    ...
    out.push({
      ...
      year:Number.isSafeInteger(row?.y)?String(row.y):'',
      genres:[],runtime:'',episodes:[]
    });
  }
  return out;
}
...
export class Catalog {
  ...
  async search({ type = 'movie', q = '', skip = 0, genre = '', feed = 'popular' } = {}) {
    ...
    if(type==='all'){
      ...
      const byId=new Map();
      const cardScore=x=>(x.poster?8:0)+(x.year?2:0)+(x.description?2:0)+(x.genres?.length||0);
    ...
    const normalize=(data,localGenre='')=>{
      ...
      for(const row of data.metas.slice(0,200)){
        ...
        try{const m=normalizeMeta(row,type,rowId);m.episodes=[];if(localGenre&&!m.genres.some(g=>g.toLowerCase()===localGenre.toLowerCase()))continue;metas.push(m);seen.add(rowId);}catch{}
      }
    ...
    const readFeed=async selected=>{
      ...
      if(catalogId==='year'){params.push('genre='+new Date().getUTCFullYear());localGenre=genre;}else if(genre)params.push('genre='+encodeURIComponent(genre));
      ...
      return this.remember(path,async()=>{
        let normalized;
        if(q){
          const primary=new URL(path,CATALOG_ORIGIN).href,secondary=new URL('/'+catalogId+path,CATALOG_SECONDARY).href;
