  ...
  async search({ type = 'movie', q = '', skip = 0, genre = '', feed = 'popular' } = {}) {
    ...
    for (const candidate of order) {
      try {
        ...
        if (result.metas.length || candidate === order.at(-1)) return { ...result, requestedFeed: feed, fallback: candidate !== feed };
      } catch (e) { lastError = e; }
    }
