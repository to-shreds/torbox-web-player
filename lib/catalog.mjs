...
function suggestionMetas(data,type,query){
  ...
  for(const row of data.d.slice(0,40)){
    ...
    seen.add(id);
    out.push({id,type,name,description:'',poster:'https://images.metahub.space/poster/medium/'+id+'/img',year:Number.isSafeInteger(row?.y)?String(row.y):'',genres:[],runtime:'',episodes:[]});
  }
  return out;
}
...
export class Catalog {
  ...
  async requestUrl(input, timeoutMs = 12000) {
    ...
    try {
      ...
      for (let hop = 0; hop < 2; hop++) {
        const allowed = url.protocol === 'https:' && [new URL(CATALOG_ORIGIN).hostname,new URL(CATALOG_SECONDARY).hostname].includes(url.hostname) && !url.username && !url.password && !url.port;
  ...
  async suggestion(type, query) {
    ...
    const metas = suggestionMetas(data, type, query);
    if (!metas.length) { const error = new AppError('SEARCH_EMPTY', 'No relevant title suggestions.', 404); error.searchEmpty = true; throw error; }
  ...
  async search({ type = 'movie', q = '', skip = 0, genre = '', feed = 'popular' } = {}) {
    ...
    if (type === 'all') {
      ...
      const byId = new Map();
      const cardScore = meta => (meta.poster ? 8 : 0) + (meta.year ? 2 : 0) + (meta.description ? 2 : 0) + (meta.genres?.length || 0);
      for (const result of results) for (const meta of result.metas || []) { const old = byId.get(meta.id); if (!old || cardScore(meta) > cardScore(old)) byId.set(meta.id, meta); }
      const needle = searchText(q);
      const metas = [...byId.values()].sort((a,b) => Number(searchText(b.name) === needle) - Number(searchText(a.name) === needle) || Number(searchText(b.name).startsWith(needle)) - Number(searchText(a.name).startsWith(needle)));
      ...
      return this.remember(path, async () => {
        ...
        if (q) {
          ...
          const attempt = async url => {
            const value = normalize(await this.requestUrl(url, 5000), localGenre);
          ...
          try { normalized = await Promise.any([attempt(primary), attempt(secondary), this.suggestion(type, q)]); }
          catch (error) {
            const reasons = error?.errors || [];
            if (reasons.some(reason => reason?.searchEmpty)) normalized = { metas: [], rawCount: 0 };
