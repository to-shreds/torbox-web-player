...
import { MAX_SOURCES, normalizeSources, targetOf, cleanText, browserContainerHints } from '../public/source-client.js';
...
export function sourceIdentityFit(meta, target, source = {}) {
  const metaYears = yearsIn(meta?.year);
  const allowedYears = new Set(metaYears);
  if (target?.type === 'series' && metaYears.length >= 2) {
    const start = Math.min(...metaYears), end = Math.max(...metaYears);
    if (end >= start && end - start <= 80) for (let year = start; year <= end; year++) allowedYears.add(year);
  }
  if (target?.type === 'series' && metaYears.length < 2) {
    const floor = metaYears.length ? metaYears[0] : 1900;
    const ceiling = new Date().getUTCFullYear() + 1;
    for (const episode of Array.isArray(meta?.episodes) ? meta.episodes : []) {
      if (episode?.season !== target.season || !episode?.released) continue;
      const year = new Date(episode.released).getUTCFullYear();
      if (Number.isSafeInteger(year) && year >= floor && year <= ceiling) allowedYears.add(year);
    }
  }
  if (!allowedYears.size) return null;
  const text = [source?.title, source?.filename].filter(Boolean).join(' ').replace(/[._-]+/g, ' ');
  if (!text) return null;
  const marker = /\b(?:s\d{1,3}(?:\s*e\d{1,4})?|season\s+\d+|\d{3,4}p|4k|web(?:\s*dl|rip)?|bluray|bdrip|hdtv|dvdrip|remux|x26[45]|h26[45]|hevc|av1)\b/i.exec(text);
  const prefix = text.slice(0, marker ? marker.index : Math.min(text.length, 180));
  const statedYears = yearsIn(prefix);
  if (!statedYears.length) return null;
  return statedYears.some(year => allowedYears.has(year));
}
function episodeTitleKey(value) {
  return cleanText(value, 250).toLowerCase()
    .replace(/\(\s*(?:part\s*)?\d+\s*\)\s*$/i, '')
    .replace(/\bpart\s*\d+\b/gi, '')
    .replace(/[^a-z0-9]+/g, ' ').trim();
}
export function sourceEpisodeTitleFit(meta, target, source = {}) {
  if (target?.type !== 'series' || !Array.isArray(meta?.episodes)) return null;
  const selected = meta.episodes.find(episode => episode?.season === target.season && episode?.episode === target.episode);
  const wanted = episodeTitleKey(selected?.name);
  if (wanted.length < 4) return null;
  const text = episodeTitleKey([source?.filename, source?.title].filter(Boolean).join(' '));
  if (!text) return null;
  if (text.includes(wanted)) return true;
  const seen = new Set([wanted]);
  for (const episode of meta.episodes) {
    if (episode?.season !== target.season) continue;
    const other = episodeTitleKey(episode?.name);
    if (other.length < 4 || seen.has(other)) continue;
    seen.add(other);
    if (text.includes(other)) return false;
  }
  return null;
}
function matchingVideos(files, target, source = {}) {
  const videoFiles = (Array.isArray(files) ? files : []).filter(f => !/(?:^|[ ._-])(sample|trailer|featurette)(?:[ ._-]|$)/i.test(fileName(f.title)));
  if (target.type !== 'series') return videoFiles;
...
export function cachedBrowserFit(cacheRow, target, source = {}) {
  if (!cacheRow || cacheRow === true || !Array.isArray(cacheRow.files) || !cacheRow.files.length) return null;
  const files = cacheRow.files.slice(0, 2000).map((file, index) => ({
    id: `cache:${index}`,
    title: cleanText(file?.name || file?.short_name, 700),
    mime: cleanText(file?.mime || file?.mimetype, 80)
  })).filter(file => file.title);
  const candidates = matchingVideos(files, target, source);
  if (!candidates.length) return null;
  let unknown = false;
  for (const file of candidates) {
    const hints = browserContainerHints({ filename: file.title, mime: file.mime });
    if (hints.browserContainer) return true;
    if (!hints.browserUnsupported) unknown = true;
  }
...
export class TorrentGateway {
  ...
  async cached(hashes, { listFiles = false } = {}) {
    if (!hashes.length) return {};
    const data = await this.call('checkcached', { params: [...hashes.map(hash => ['hash', hash]), ['format', 'object'], ['list_files', listFiles ? 'true' : 'false']] });
    ...
    for (const hash of hashes) {
      const row = data[hash];
      const present = row === true || (!!row && typeof row === 'object');
      if (!listFiles) {
        result[hash] = present;
        continue;
      }
      const files = present && row !== true && Array.isArray(row.files) ? row.files.slice(0, 2000).map(file => ({
        name: cleanText(file?.name || file?.short_name, 700),
        size: Number.isSafeInteger(file?.size) && file.size >= 0 ? file.size : null,
        mime: cleanText(file?.mimetype, 80)
      })).filter(file => file.name) : [];
      result[hash] = { cached: present, files };
    }
    return result;
  }
  async find(hash) {
  ...
  async create(hash, onlyCached) {
    ...
    if (!validId(data?.torrent_id)) throw new AppError('CREATE_UNCERTAIN', 'TorBox accepted the request but did not return a usable identifier. Check status before retrying.', 502);
...
export class Discovery {
  ...
  async register(input, sessionId) {
    ...
    if (input.sources.length && !normalizedSources.length) throw new AppError('INVALID_SOURCES', 'No supported torrent hashes were supplied.', 400);
    const classified = normalizedSources.map(source => ({
      source,
      identityFit: sourceIdentityFit(meta, target, source),
      episodeTitleFit: sourceEpisodeTitleFit(meta, target, source)
    }));
    const accepted = classified.filter(row => row.identityFit !== false && row.episodeTitleFit !== false);
    const rejectedIdentityCount = classified.length - accepted.length;
    this.prune();
    if (this.tickets.size + accepted.length > 2000) throw new AppError('SOURCE_CAPACITY', 'Too many source selections are open. Try again later.', 429);
    let cache = {};
    const warnings = [];
    if (rejectedIdentityCount) warnings.push('Some source results were excluded because their title identity conflicts with the selected catalog title or episode.');
    try { cache = await this.gateway.cached([...new Set(accepted.map(row => row.source.hash))], { listFiles: true }); }
    catch { warnings.push('Sources were found, but TorBox availability could not be checked. Their cached status is unknown.'); }
    const rows = accepted.map(({ source, identityFit, episodeTitleFit }) => {
      const cacheRow = cache[source.hash];
      const cached = typeof cacheRow === 'boolean' ? cacheRow : cacheRow?.cached === true ? true : cacheRow?.cached === false ? false : null;
      const cachedBrowserPlayable = cached === true ? cachedBrowserFit(cacheRow, target, source) : null;
      const id = randomUUID();
      this.tickets.set(id, { source, target, sessionId, until: this.now() + 1800000 });
      return { id, hash: source.hash, title: source.title, label: source.label, provider: source.provider, providers: source.providers, filename: source.filename, quality: source.quality, hint: source.hint, size: source.size, seeders: source.seeders, releaseQuality: source.releaseQuality, container: source.container, resolution: source.resolution, cached, cachedBrowserPlayable, browserFriendly: source.browserFriendly, browserContainer: source.browserContainer, browserUnsupported: source.browserUnsupported, containerStatus: source.containerStatus, audioRisk: source.audioRisk, videoRisk: source.videoRisk, videoCodec: source.videoCodec, audioCodecs: source.audioCodecs, score: source.score + (identityFit === true ? 60 : 0) + (episodeTitleFit === true ? 90 : 0) + (cached ? 8 : 0) + (cachedBrowserPlayable === true ? 80 : cachedBrowserPlayable === false ? -120 : 0) };
    }).sort((a, b) => b.score - a.score);
    return { sources: rows, warning: warnings.join(' '), target, state: rows.length ? 'sources_found' : 'no_sources' };
  }
  ...
  async status(id, sessionId, explicitVideoId) {
    ...
    if (file) {
      const actual = browserContainerHints(file);
      const compatibility = { browserFriendly: row.source.browserFriendly && !actual.browserUnsupported, browserContainer: actual.browserContainer, browserUnsupported: actual.browserUnsupported, containerStatus: actual.containerStatus, container: actual.container, audioRisk: row.source.audioRisk, videoRisk: row.source.videoRisk, hint: actual.browserUnsupported ? `${actual.container.toUpperCase()} cannot play in Android Chrome.` : row.source.hint, videoCodec: row.source.videoCodec, audioCodecs: row.source.audioCodecs };
      if (actual.browserUnsupported) return { state: 'browser_unsupported', file, target: row.target, compatibility, message: `This source resolved to ${actual.container.toUpperCase()}, which Android Chrome cannot play. Trying another source can find an MP4 or WebM version.` };
      return { state: 'ready', file, target: row.target, compatibility, message: row.source.audioRisk ? 'This version is ready, but its Dolby/DTS audio may play silently in Chrome. Choose another version for sound, or play it anyway.' : 'The selected file is ready. Browser playback depends on its codecs.' };
    }
