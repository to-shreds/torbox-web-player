...
export class Discovery {
  ...
  async register(input, sessionId) {
    ...
    const classified = normalizedSources.map(source => ({ source, identityFit: sourceIdentityFit(meta, target, source) }));
    const accepted = classified.filter(row => row.identityFit !== false);
    ...
    let cache = {};
    const warnings = [];
    if (rejectedIdentityCount) warnings.push('Some source results were excluded because their title identity conflicts with the selected catalog title.');
    ...
    const rows = accepted.map(({ source, identityFit }) => {
      ...
      return { id, hash: source.hash, title: source.title, label: source.label, provider: source.provider, providers: source.providers, filename: source.filename, quality: source.quality, hint: source.hint, size: source.size, seeders: source.seeders, releaseQuality: source.releaseQuality, container: source.container, resolution: source.resolution, cached, cachedBrowserPlayable, browserFriendly: source.browserFriendly, browserContainer: source.browserContainer, browserUnsupported: source.browserUnsupported, containerStatus: source.containerStatus, audioRisk: source.audioRisk, videoRisk: source.videoRisk, videoCodec: source.videoCodec, audioCodecs: source.audioCodecs, score: source.score + (identityFit === true ? 60 : 0) + (cached ? 8 : 0) + (cachedBrowserPlayable === true ? 80 : cachedBrowserPlayable === false ? -120 : 0) };
