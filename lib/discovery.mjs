...
export class Discovery {
  ...
  async register(input, sessionId) {
    ...
    const rows = sources.map(source => {
      ...
      return { id, title: source.title, label: source.label, filename: source.filename, quality: source.quality, hint: source.hint, size: source.size, cached, score: source.score + (cached ? 20 : 0) };
  ...
  async status(id, sessionId, explicitVideoId) {
    ...
    if (file) return { state: 'ready', file, target: row.target, message: 'The selected file is ready. Browser playback depends on its codecs.' };
