...
export class TorrentGateway {
  ...
  async call(path, { params = [], form, creating = false } = {}) {
    ...
    try {
      ...
      if (!response.ok || payload?.success !== true) {
        ...
        // Without this, a refusal from TorBox reaches the device as a generic sentence and leaves
        // no trace at all, which is why this failure has been diagnosed by guesswork. The route,
        // status and TorBox's own error/detail are what identify the cause. No key and no torrent
        // hash are recorded; detail is bounded and may name the release, which is acceptable in the
        // account owner's own private service log.
        console.warn(JSON.stringify({
          event: 'torbox_call_failed', path, status: response.status, creating,
          error: typeof error === 'string' ? error.slice(0, 80) : null,
          detail: typeof payload?.detail === 'string' ? payload.detail.slice(0, 200) : null
        }));
      ...
      // A timeout or transport fault here is otherwise indistinguishable from a refusal.
      console.warn(JSON.stringify({ event: 'torbox_call_unreachable', path, creating, reason: e?.name || 'unknown' }));
  ...
  async cached(hashes) {
    ...
    // An empty array is TorBox saying "none of these are cached", not an unreadable response.
    if (Array.isArray(data) && !data.length) return Object.fromEntries(hashes.map(hash => [hash, false]));
    if (!data || typeof data !== 'object' || Array.isArray(data)) {
      console.warn(JSON.stringify({ event: 'torbox_cache_schema', requested: hashes.length, shape: responseShape(data) }));
      throw new AppError('CACHE_SCHEMA', 'TorBox returned an unreadable cache response.');
    }
    // The returned map is not guaranteed to key hashes in the casing they were sent. Every other
    // hash comparison in this file already normalizes case; this lookup did not, so a casing
    // difference reported every source as uncached, silently, and sent ordinary Play down the
    // download path instead of playing an already-cached version.
    const rows = new Map(Object.entries(data).map(([key, value]) => [key.toLowerCase(), value]));
    const result = {};
    let cachedCount = 0;
    for (const hash of hashes) {
      const row = rows.get(hash.toLowerCase());
      const isCached = row === true || (!!row && typeof row === 'object');
      if (isCached) cachedCount++;
      result[hash] = isCached;
    }
    // Cache availability decides whether Play is instant or a download, so its outcome is worth one
    // line. Counts only: no hashes are recorded.
    console.warn(JSON.stringify({ event: 'torbox_cache_checked', requested: hashes.length, returned: rows.size, cached: cachedCount }));
    return result;
  }
  async find(hash) {
...
export class Discovery {
  ...
  async status(id, sessionId, explicitVideoId) {
    ...
    if (!candidates.length) {
      // Episode matching relies on parsing SxxEyy (or 1x02) out of each file name, or on an exact
      // match against the source's own filename. A pack that names episodes any other way matches
      // nothing and looks like a lookup failure. Numbers only: no file names are recorded.
      console.warn(JSON.stringify({
        event: 'torbox_episode_unmatched', type: row.target.type,
        want: row.target.type === 'series' ? `${row.target.season}x${row.target.episode}` : null,
        videos: files.length, sourceFilename: !!row.source.filename,
        parsed: files.slice(0, 8).map(f => { const e = episodeIdentity(f.title); return e ? `${e.season}x${e.episode}${e.multi ? '+' : ''}` : null; })
      }));
      return { state: 'unavailable', message: 'No video could be confidently matched to the selected title or episode. Choose another source.' };
    }
