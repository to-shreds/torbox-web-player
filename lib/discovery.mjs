...
export class Discovery {
  ...
  async register(input, sessionId) {
    ...
    let cache = {}, warning = '';
    try { cache = await this.gateway.cached([...new Set(sources.map(s => s.hash))], { listFiles: true }); }
    ...
    const rows = sources.map(source => {
      ...
      return { id, hash: source.hash, title: source.title, label: source.label, provider: source.provider, providers: source.providers, filename: source.filename, quality: source.quality, hint: source.hint, size: source.size, seeders: source.seeders, releaseQuality: source.releaseQuality, container: source.container, resolution: source.resolution, cached, cachedBrowserPlayable, browserFriendly: source.browserFriendly, browserContainer: source.browserContainer, browserUnsupported: source.browserUnsupported, containerStatus: source.containerStatus, audioRisk: source.audioRisk, videoRisk: source.videoRisk, videoCodec: source.videoCodec, audioCodecs: source.audioCodecs, score: source.score + (cached ? 8 : 0) + (cachedBrowserPlayable === true ? 80 : cachedBrowserPlayable === false ? -120 : 0) };
