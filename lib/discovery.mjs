...
export class Discovery {
  ...
  async register(input, sessionId) {
    ...
    const rows = sources.map(source => {
      ...
      return { id, title: source.title, label: source.label, filename: source.filename, quality: source.quality, hint: source.hint, size: source.size, cached, browserFriendly: source.browserFriendly, audioRisk: source.audioRisk, videoRisk: source.videoRisk, videoCodec: source.videoCodec, audioCodecs: source.audioCodecs, score: source.score + (cached ? 8 : 0) };
