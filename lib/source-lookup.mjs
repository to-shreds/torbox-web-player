...
export const STREMTHRU_TORZ_ORIGIN = 'https://stremthru.elfhosted.com/stremio/torz/eyJzdG9yZXMiOlt7ImMiOiJwMnAiLCJ0IjoiIn1dfQ==';
export const PUBLIC_STREMIO_PROVIDERS = Object.freeze([
  Object.freeze({ id: 'stremthru', name: 'StremThru Torz', origin: STREMTHRU_TORZ_ORIGIN }),
...
class CachedProvider {
  constructor({ fetchFn = fetch, now = Date.now, timeoutMs = 35000, name }) {
    Object.assign(this, { fetchFn, now, timeoutMs, name });
    this.cache = new Map();
  ...
  cachedLookup(input, requestFn) {
    ...
    if (this.now() < this.cooldownUntil) return Promise.reject(new SourceLookupError('SOURCE_PROVIDER_COOLDOWN', this.name + ' is temporarily cooling down.', 503));
    ...
    const operation = requestFn(selected).then(sources => {
      ...
      this.remember(key, { value, until: this.now() + (sources.length ? 15 * 60000 : 120000) });
      return value;
    }).catch(error => {
      this.lastOutcome = error?.code || 'error';
      this.remember(key, { error, until: this.now() + 30000 });
      throw error;
...
export class SourceLookup extends CachedProvider {
  constructor({ fetchFn = fetch, now = Date.now, timeoutMs = 35000, provider = 'zilean' } = {}) {
    super({ fetchFn, now, timeoutMs, name: 'Zilean' });
...
export class MultiSourceLookup {
  constructor({ fetchFn = fetch, now = Date.now, timeoutMs = 30000, providers } = {}) {
    this.now = now;
    this.cache = new Map();
    this.inflight = new Map();
    this.providers = providers || [
      new SourceLookup({ fetchFn, now, timeoutMs: Math.min(timeoutMs, 20000) }),
      ...PUBLIC_STREMIO_PROVIDERS.map(row => new StremioSourceLookup({ ...row, fetchFn, now, timeoutMs }))
    ];
  }
  ...
  async run(selected) {
    ...
    for (let index = 0; index < this.providers.length; index++) {
      ...
      try {
        const result = await provider.lookup(selected);
        successfulProviders++;
        if (result.sources.length) {
          groups.push(result.sources);
          providersUsed.push(name);
        }
        const merged = mergeProviderSources(groups);
        if ((index === 0 && merged.length >= PRIMARY_ENOUGH) || merged.length >= TARGET_SOURCE_COUNT) {
          return {
            sources: merged,
            provider: providersUsed.join(' + ') || name,
        ...
        failures.push({ name, code: error?.code || 'SOURCE_UNAVAILABLE' });
      }
    }

    const sources = mergeProviderSources(groups);
    if (!successfulProviders && failures.length === this.providers.length) {
    ...
    return {
      ...
      warning: failures.length ? 'Some backup source indexes were unavailable, but the search completed with the remaining providers.' : ''
    };
  }
}
