...
export class MultiSourceLookup {
  ...
  async run(selected) {
    const groups=[],providersTried=[],providersUsed=[],failures=[];
    let successfulProviders=0,settled=0,done=false,graceTimer=null,hardTimer=null;
    return await new Promise((resolve,reject)=>{
      const finish=()=>{
        if(done)return;done=true;if(graceTimer)clearTimeout(graceTimer);if(hardTimer)clearTimeout(hardTimer);
        const merged=mergeProviderSources(groups);
        if(!merged.length){
          if(successfulProviders>0)return resolve({sources:[],provider:'Anonymous source indexes',providers:[],providersTried:[...providersTried],fallbackUsed:false,warning:failures.length?'Some anonymous source indexes were unavailable.':''});
          const code=settled<this.providers.length?'SOURCE_TIMEOUT':'SOURCE_ALL_UNAVAILABLE';
          return reject(new SourceLookupError(code,code==='SOURCE_TIMEOUT'?'Source lookup took too long. Try again shortly.':'All configured anonymous source indexes are temporarily unavailable. No torrent has been added.',code==='SOURCE_TIMEOUT'?504:503));
        }
        const fallbackNames=new Set(this.providers.slice(this.primaryCount).map(provider=>provider.name||'Source provider'));
        resolve({
          sources:merged,
          provider:providersUsed.join(' + ')||'Anonymous source indexes',
          providers:[...providersUsed],
          providersTried:[...providersTried],
          fallbackUsed:providersUsed.some(name=>fallbackNames.has(name)),
      ...
      const schedule=ms=>{if(graceTimer)return;graceTimer=setTimeout(finish,ms);};
      const maybeFinish=()=>{
        const merged=mergeProviderSources(groups);
        if(merged.length>=TARGET_SOURCE_COUNT)return schedule(120);
        if(merged.length&&successfulProviders>=2)return schedule(300);
        if(settled===this.providers.length)finish();
      };
      hardTimer=setTimeout(finish,5500);
      for(const provider of this.providers){
        const name=provider.name||'Source provider';providersTried.push(name);
        Promise.resolve().then(()=>provider.lookup(selected)).then(value=>{
          if(done)return;settled++;successfulProviders++;
          const rows=value?.sources||[];if(rows.length){groups.push(rows);providersUsed.push(name);}
          maybeFinish();
        },error=>{
          if(done)return;settled++;failures.push({name,code:error?.code||'SOURCE_UNAVAILABLE'});maybeFinish();
        });
      }
    });
  }}
