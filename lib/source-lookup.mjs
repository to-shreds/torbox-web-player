...
const INCOMPLETE_RESULT = Symbol('incomplete source result');
...
export function normalizeTorznabXml(xml, input, providerName = 'MediaFusion Torznab') {
  ...
  for (const item of items.slice(0, 500)) {
    ...
    const rawImdb = String(attrs.imdb || '').trim().toLowerCase();
    const imdb = /^tt[0-9]{5,12}$/.test(rawImdb) ? rawImdb : /^[0-9]{5,12}$/.test(rawImdb) ? 'tt' + rawImdb : '';
    // Torznab is a broad search surface. Require the result itself to attest the requested IMDb identity.
    if (imdb !== selected.id) continue;
...
export class TorrentioSourceLookup extends CachedProvider {
  constructor({ fetchFn = fetch, now = Date.now, timeoutMs = 12000 } = {}) {
    super({ fetchFn, now, timeoutMs, name: 'Torrentio' });
    this.origin = TORRENTIO_ORIGIN;
  }
  ...
  async request(selected) {
    const resourceId = selected.type === 'series' ? selected.id + ':' + selected.season + ':' + selected.episode : selected.id;
    const url = new URL('/sort=qualityseed/stream/' + selected.type + '/' + resourceId + '.json', this.origin);
    let response;
    try {
      response = await this.fetchFn(url, {
        method: 'GET',
      ...
      if (response.status === 429) {
        this.cooldownUntil = this.now() + retryDelay(response, this.now, 60000);
        throw new SourceLookupError('SOURCE_RATE_LIMITED', 'Torrentio is rate limiting source queries.', 429);
      }
      if ([401, 403].includes(response.status)) {
        this.cooldownUntil = this.now() + 30 * 60000;
        throw new SourceLookupError('SOURCE_ACCESS_DENIED', 'Torrentio declined this anonymous source query.');
      }
      if (response.status >= 300 && response.status < 400) {
        this.cooldownUntil = this.now() + 30 * 60000;
        throw new SourceLookupError('SOURCE_REQUIRES_CONFIGURATION', 'Torrentio redirected instead of returning anonymous stream metadata.');
      }
      if (!response.ok) {
        this.cooldownUntil = this.now() + (response.status >= 500 ? 5 * 60000 : 30 * 60000);
        throw new SourceLookupError('SOURCE_UNAVAILABLE', 'Torrentio is unavailable (HTTP ' + response.status + ').');
      }
      return normalizeStremioStreams(await readJson(response, 4 * 1024 * 1024), selected, this.name);
    } catch (e) {
      if (e instanceof SourceLookupError) throw e;
      if (['TimeoutError', 'AbortError'].includes(e?.name)) {
        this.cooldownUntil = this.now() + 5 * 60000;
        throw new SourceLookupError('SOURCE_TIMEOUT', 'Torrentio timed out.', 504);
      }
      this.cooldownUntil = this.now() + 5 * 60000;
      throw new SourceLookupError('SOURCE_UNAVAILABLE', 'Torrentio could not be reached.');
    } finally {
      try { if (!response?.bodyUsed) await response?.body?.cancel(); } catch {}
    }
  }
...
export class MultiSourceLookup {
  constructor({ fetchFn = fetch, now = Date.now, timeoutMs = 10000, providers, primaryCount } = {}) {
    this.now = now;
    this.timeoutMs = Math.max(1, Number(timeoutMs) || 10000);
    this.cache = new Map();
    this.inflight = new Map();
    const supplied = Array.isArray(providers);
    this.providers = supplied ? providers : [
      new SourceLookup({ fetchFn, now, timeoutMs: this.timeoutMs }),
      new TorrentioSourceLookup({ fetchFn, now, timeoutMs: this.timeoutMs }),
      new TorznabSourceLookup({ fetchFn, now, timeoutMs: this.timeoutMs }),
      ...ACTIVE_STREMIO_PROVIDERS.map(row => new StremioSourceLookup({ ...row, fetchFn, now, timeoutMs: this.timeoutMs }))
    ];
    this.primaryCount = Math.max(1, Math.min(primaryCount ?? (supplied ? 1 : 3), this.providers.length));
  }
  ...
  async lookup(input) {
    ...
    const operation = this.run(selected).then(value => {
      const ttl = value[INCOMPLETE_RESULT] ? 15000 : value.sources.length ? 15 * 60000 : 60000;
      this.remember(key, { value, until: this.now() + ttl });
      return value;
    }).catch(error => {
      this.remember(key, { error, until: this.now() + 15000 });
      throw error;
  ...
  async run(selected) {
    const groups = [];
    const providersTried = this.providers.map(provider => provider.name || 'Source provider');
    const providersUsed = [];
    const failures = [];
    let successfulProviders = 0, providersWithSources = 0, settled = 0, fallbackUsed = false, done = false, graceTimer, hardTimer;
    return await new Promise((resolve, reject) => {
      const finish = () => {
        if (done) return;
        done = true;
        clearTimeout(graceTimer); clearTimeout(hardTimer);
        const merged = mergeProviderSources(groups), incomplete = settled < this.providers.length;
        if (!merged.length) {
          if (successfulProviders && !incomplete) return resolve({ sources: [], provider: 'Anonymous source indexes', providers: [], providersTried, fallbackUsed: false, warning: failures.length ? 'Some anonymous source indexes were unavailable.' : '' });
          const timedOut = incomplete || failures.some(row => row.code === 'SOURCE_TIMEOUT');
          return reject(new SourceLookupError(timedOut ? 'SOURCE_TIMEOUT' : 'SOURCE_ALL_UNAVAILABLE', timedOut ? 'Source lookup took too long. Try again shortly.' : 'All configured anonymous source indexes are temporarily unavailable. No torrent has been added.', timedOut ? 504 : 503));
        }
        const result = {
          sources: merged,
          ...
          providersTried,
          fallbackUsed,
          warning: failures.length || incomplete ? 'Some anonymous source indexes were unavailable, but the search completed with the remaining providers.' : ''
        };
        if (incomplete) result[INCOMPLETE_RESULT] = true;
        resolve(result);
      };
      const schedule = ms => { if (!graceTimer) graceTimer = setTimeout(finish, ms); };
      const maybeFinish = () => {
        const merged = mergeProviderSources(groups), browserCandidates = browserSourceCount(merged), plausibleCandidates = merged.some(source => source?.browserUnsupported !== true);
        if (settled === this.providers.length) return finish();
        if (merged.length >= TARGET_SOURCE_COUNT && browserCandidates) return schedule(150);
        if ((browserCandidates && successfulProviders >= 2) || (providersWithSources >= 2 && plausibleCandidates)) return schedule(600);
      };
      hardTimer = setTimeout(finish, this.timeoutMs);
      this.providers.forEach((provider, index) => {
        const name = provider.name || 'Source provider';
        Promise.resolve().then(() => provider.lookup(selected)).then(value => {
          if (done) return;
          settled++; successfulProviders++;
          const sources = value?.sources || [];
          if (sources.length) {
            groups.push(sources); providersUsed.push(name); providersWithSources++;
            if (index >= this.primaryCount) fallbackUsed = true;
          }
          maybeFinish();
        }, error => {
          if (done) return;
          settled++; failures.push({ name, code: error?.code || 'SOURCE_UNAVAILABLE' }); maybeFinish();
        });
      });
    });
  }
