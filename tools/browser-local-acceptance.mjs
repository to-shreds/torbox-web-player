...
const engines={chrome:{engine:chromium,channel:'chrome'},firefox:{engine:firefox},webkit:{engine:webkit}};
try{for(const [name,{engine,channel}] of Object.entries(engines)){
 const browser=await engine.launch({headless:true,...(channel?{channel}:{})});try{
 ...
 await page.locator('#portable-password').fill('fixture-transfer-password');assert.equal(await page.locator('#portable-qr').isVisible(),false,'An old unprotected QR is removed when adding a password');const downloadEvent=page.waitForEvent('download');await page.locator('#portable-export-file').click();const download=await downloadEvent,file='.release-results/'+name+'.twsetup';await download.saveAs(file);
 ...
 assert.deepEqual(a.errors,[]);assert.deepEqual(b.errors,[]);results.push({engine:name,version:browser.version(),passed:true,checks:['simple onboarding','automatic Render failure fallback','search keyboard blur','actual QR read/import','password-protected file import','stale QR removal','metadata hydration','old bookmark redirect','state preservation','mobile layout','source lookup prefix','synthetic H264/AAC playback']});console.log(name+': all browser acceptance checks passed');
